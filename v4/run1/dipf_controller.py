"""
FULL PURE-DIPF CONTROLLER - OPTIMIZATION VERSION (v4)

This directory's sole purpose is running optimize.py: a Differential
Evolution search over (alpha, tau_y_base, sx, delta_lane, lambda_obs) that
minimizes the average episode cost C (see compute_episode_cost()) across a
fixed batch of seeds -- implementing the "Scenario-Specific Parameter
Optimization" algorithm (DE/best/2 mutation + exponential crossover +
greedy selection) rather than v3's Bayesian (Optuna/TPE) search. tau_x_base
is held fixed at its config.yaml value. The controller itself, and its
env-var hooks, are otherwise unchanged from v3.
...
def compute_episode_cost(hist):
    ...
    c_total = 0.0 * c_collision + 20.0 * c_dist + c_time
...
def run_simulation():
    ...
    print("=" * 90)
    print("FULL PURE-DIPF CONTROLLER (v4 -- differential evolution optimization variant)")
    print("=" * 90)
