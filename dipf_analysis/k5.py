...
def kprime(dx, dy, dvx, dvy):
    ...
    dvx=np.where(np.abs(dvx) < eps, eps, dvx)
    dvy=np.where(np.abs(dvy) < eps, eps, dvy)
    dist = np.sqrt(((dx+alpha*dvx) / (tx)) ** 2 + ((dy+alpha*dvy) / (ty)) ** 2)
