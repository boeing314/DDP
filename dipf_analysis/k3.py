...
N_LEVELS = 20            # many, finely-spaced levels -> the "fan" pattern near the vehicle
...
def kprime(dx, dy, dvx, dvy):
    ...
    dxx=np.where(dx>w,w-dx,np.where(dx<-w,dx+w,0))
    dyy=np.where(dy>l,l-dy,np.where(dy<-l,dy+l,0))
    dist = np.sqrt((dxx / tx) ** 2 + (dyy / ty) ** 2)
