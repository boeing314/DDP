...
\begin{figure}[H]
    ...
    \label{fig:base-vel}
\end{figure}
The force increases with increase in receding velocity at a fixed point. In an ideal scenario the force should decrease when the vehicles are receding.
\subsection{More free parameters than DOF}

The model introduces 4 free parameters ($\lambda$, $\alpha$, $\tau_x$,
$\tau_y$, $w$, $l$, $\Delta v_{max}$) while the degree of freedom of the model is only 3. So reduction of parameters will help in the optimization problem.
